...
function validateInput(text: string): { isValid: boolean;
    ...
    if (text.length < CONFIG.MIN_TEXT_LENGTH) {
        return {
            isValid : false,
            error   : `Text must be at least ${CONFIG.MIN_TEXT_LENGTH} characters long`
        };
    }
...
function extractRoleTitle(text: string): string {
    ...
    for (let i = 0; i < Math.min(5, lines.length); i++) {
        ...
        if (/^(location|salary|experience|company|description|cdi|cdd|télétravail)/i.test(cleanLine)) {
            continue;
        }

        // Check if line looks like a job title (contains common job-related keywords)
        if (
            /(developer|engineer|architect|manager|analyst|specialist|consultant|lead|senior|junior|mid|full.?stack|frontend|backend|devops|cloud|data|software|web|mobile|ui|ux|qa|test)/i.test(
                cleanLine
            )
        ) {
            return cleanLine;
...
function extractTechnologies(text: string, contextKeywords: string[]): string[] {
    ...
    for (const pattern of techPatterns) {
        ...
        if (matches) {
            technologies.push(...matches.map((m) => {
                return m.trim();
            }));
        }
    }
...
export type {
    TechStackExtractionResult, StackSelectionJson
};
