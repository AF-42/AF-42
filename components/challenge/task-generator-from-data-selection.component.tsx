...
export function TaskGeneratorFormFromDataSelection() {
    ...
    const extractAndMergeTechStack = async (
        formattedText: string,
        existingJsonConfig: string,
    ) => {
        ...
        try {
            ...
            if (result.success && result.techStack) {
                ...
                setTechStackError(
                    result.error || 'Failed to extract tech stack',
                );
    ...
    const handleTextExtracted = async (result: TextExtractionResult) => {
        if (result.success) {
            ...
            try {
                ...
                const techStack = await extractAndMergeTechStack(
                    formattedText,
                    jsonConfig,
                );
            } catch (error) {
    ...
    async function handleSubmit(formData: z.infer<typeof formSchema>) {
        ...
        const res = await createTechChallenge(
            jobOfferContent,
            jsonConfig || '',
        );
        setResult(res);
    ...
    return (
        <>
                ...
                >
                    <FormItem>
                        ...
                        <FormDescription>
                            ...
                            This JSON config is automatically generated from
                            your selections above and tech stack extraction from
                            translated text. It will be used to generate the
                            challenge.
                        </FormDescription>
                            ...
                            <div className='min-h-[200px] border rounded-lg p-4 bg-muted/50'>
                                {jsonConfig ? (
                                    ...
                                    <div className='flex items-center justify-center h-full text-muted-foreground'>
                                        Select items from the data selection
                                        component above to generate JSON config
                                    </div>
                                )}
                            </div>
                        </FormControl>
            ...
            <div className='w-full mt-4 border rounded-lg p-4 bg-muted/50'>
                <h1 className='text-2xl text-green-600 font-bold pb-4'>
                    Result:
                </h1>
                <pre className='text-lg font-mono overflow-auto	whitespace-pre-wrap mt-4'>
                    {result}
                </pre>
            </div>
        </>
